...
*/
(function(d){var a,b,c;c=function(e,f){return function(){return e.apply(f,arguments)}};b={align:"center",container:d("body"),offset:2,autoResize:false,itemWidth:0,flexibleWidth:0,resizeDelay:50,onLayoutChanged:undefined};a=(function(f){function e(h,g){this.handler=h;this.columns=null;this.containerWidth=null;this.resizeTimer=null;this.direction="left";d.extend(true,this,b,g);this.update=c(this.update,this);this.onResize=c(this.onResize,this);this.getItemWidth=c(this.getItemWidth,this);this.layout=c(this.layout,this);this.layoutFull=c(this.layoutFull,this);this.layoutColumns=c(this.layoutColumns,this);this.clear=c(this.clear,this);if(this.autoResize){d(window).bind("resize.wookmark",this.onResize);this.container.bind("refreshWookmark",this.onResize)}}e.prototype.update=function(g){d.extend(true,this,g)};e.prototype.onResize=function(){clearTimeout(this.resizeTimer);this.resizeTimer=setTimeout(this.layout,this.resizeDelay)};e.prototype.getItemWidth=function(){var l=this.itemWidth,k=this.container.width(),i=this.handler.eq(0);if(this.itemWidth===undefined||this.itemWidth===0&&!this.flexibleWidth){l=i.outerWidth()}else{if(typeof this.itemWidth=="string"&&this.itemWidth.indexOf("%")>=0){l=parseFloat(this.itemWidth)/100*k}}if(this.flexibleWidth){var g=this.flexibleWidth;if(typeof g=="string"&&g.indexOf("%")>=0){g=parseFloat(g)/100*k-i.outerWidth()+i.innerWidth()}var h=Math.floor(1+k/(g+this.offset)),j=(k-(h-1)*this.offset)/h;l=Math.max(l,Math.floor(j));this.handler.css("width",l)}return l};e.prototype.layout=function(){if(!this.container.is(":visible")){return}var j=this.getItemWidth()+this.offset,i=this.container.width(),g=Math.floor((i+this.offset)/j),k=0,h=0;g=Math.min(g,this.handler.length);if(this.align=="left"||this.align=="right"){k=Math.floor((g/j+this.offset)/2)}else{k=Math.round((i-(g*j-this.offset))/2)}this.direction=this.align=="right"?"right":"left";if(this.columns!=null&&this.columns.length==g){h=this.layoutColumns(j,k)}else{h=this.layoutFull(j,g,k)}this.container.css("height",h);if(this.onLayoutChanged!==undefined&&typeof this.onLayoutChanged==="function"){this.onLayoutChanged()}};e.prototype.layoutFull=function(m,p,s){var y,x,n,t=0,q=0,r=0,l=this.handler.length,v=null,g=null,o={position:"absolute"},h,w=[],u=this.align=="left"?true:false;this.columns=[];while(w.length<p){w.push(0);this.columns.push([])}for(;t<l;t++){y=this.handler.eq(t);v=w[0];g=0;for(q=0;q<p;q++){if(w[q]<v){v=w[q];g=q}}if(g==0&&u){h=0}else{h=g*m+s}o[this.direction]=h;o.top=v;y.css(o);w[g]+=y.outerHeight()+this.offset;this.columns[g].push(y)}return Math.max.apply(Math,w)};e.prototype.layoutColumns=function(h,n){var p=[],o=0,m=0,j,q,l,g;for(;o<this.columns.length;o++){p.push(0);j=this.columns[o];g=o*h+n;for(m=0;m<j.length;m++){q=j[m];l={top:p[o]};l[this.direction]=g;q.css(l);p[o]+=q.outerHeight()+this.offset}}return Math.max.apply(Math,p)};e.prototype.clear=function(){clearTimeout(this.resizeTimer);d(window).unbind("resize.wookmark",this.onResize);this.container.unbind("refreshWookmark",this.onResize)};return e})();d.fn.wookmark=function(e){if(!this.wookmarkInstance){this.wookmarkInstance=new a(this,e||{})}else{this.wookmarkInstance.update(e||{})}this.wookmarkInstance.layout();return this.show()}})(jQuery);